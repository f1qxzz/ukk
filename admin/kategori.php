...
if (isset($_POST['add'])) {
    ...
    $msg = $s->execute()?'Kategori ditambahkan!':'Gagal!'; $msgType='success'; $s->close();
}
if (isset($_POST['edit'])) {
    ...
    $msg=$s->execute()?'Kategori diperbarui!':'Gagal!'; $msgType='success'; $s->close();
}
if (isset($_POST['delete'])) {
    ...
    if($chk>0){ $msg='Kategori masih digunakan oleh buku!'; $msgType='warning'; }
        ...
        $s->bind_param("i",$id);
        $msg=$s->execute()?'Kategori dihapus!':'Gagal!'; $msgType='success'; $s->close();
    }
}
