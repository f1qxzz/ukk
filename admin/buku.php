...
if (isset($_POST['add'])) {
    ...
    if (!$uploadGagal) {
        // s=judul, i=id_kat, s=peng, s=nerbit, i=tahun, s=isbn, s=desk, i=stok, s=status, s=cover → 10 param
        $s = $conn->prepare(
...
if (isset($_POST['edit'])) {
    ...
    // Cek file baru
    $adaFileBaru = isset($_FILES['cover'])
                   && $_FILES['cover']['error'] === UPLOAD_ERR_OK
                   && $_FILES['cover']['size'] > 0;

    $newCover    = $oldCover; // default pertahankan lama
    $uploadGagal = false;
    ...
    if ($adaFileBaru) {
        ...
        if (!$coverResult['ok']) {
            ...
            // Hapus file lama, pakai file baru
    ...
    if (!$uploadGagal) {
        // s=judul, i=id_kat, s=peng, s=nerbit, i=tahun, s=isbn, s=desk, i=stok, s=status, s=cover, i=id → 11 param
        $s = $conn->prepare(
...
$s->bind_param(
    ...
    $status, $newCover, $id
        ...
        if ($s->execute()) {
            ...
            if ($adaFileBaru) deleteBookCover($newCover);
            $msg     = 'Gagal memperbarui buku: ' . $conn->error;
            $msgType = 'danger';
        }
        $s->close();
    }
